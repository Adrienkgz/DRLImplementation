...
class DDPGTrainer:
    def __init__(self, env, args, device: torch.device | None = None):
        ...
        self.qnetwork = QNetwork(env).to(self.device)
        
        # Initialize target networks
        self.target_actor = Actor(env).to(torch.device(self.device))
        self.target_actor.load_state_dict(self.actor.state_dict())
        self.target_q_network = QNetwork(env).to(torch.device(self.device))
        self.target_q_network.load_state_dict(self.qnetwork.state_dict())
        
        # Initialize optimizers
        self.actor_optimizer = torch.optim.Adam(self.actor.parameters(), lr=args.learning_rate)
        self.critic_optimizer = torch.optim.Adam(self.qnetwork.parameters(), lr=args.learning_rate)
    ...
    def update_critic(self, obs_b, action_b, reward_b, done_b, next_obs_b):
        with torch.no_grad():
            ...
            # Compute the target Q value using the target Q network and the next action
            # Formula: Q(s,a) = r + γ * Q(s', a')
            target_q = reward_b + (1 - done_b) * self.args.gamma * self.target_q_network(next_obs_b, next_action).squeeze(1)
                        
        loss = nn.MSELoss()(self.qnetwork(obs_b, action_b).squeeze(), target_q)
    ...
    def update_actor(self, obs_b):
        ...
        actor_loss = -self.qnetwork(obs_b.to(self.device), self.actor(obs_b.to(self.device))).mean()
        ...
        self.soft_update(self.target_q_network, self.qnetwork)
...
if __name__ == '__main__':
    ...
    ddpg = DDPG('MlpPolicy', env, verbose=1, tensorboard_log="./ddpg_pendulum_tensorboard/")
    ddpg = DDPGTrainer(env=env, args=args, device=torch.device('cpu'))
    ddpg.learn(10000000)